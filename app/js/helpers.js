...
function lglStringify(obj, space = null, ignoreCircular = true) {
  const cache = [];
...
async function lglHandleRes(fetchRes, onStream, debugName = null) {
  if (!onStream) {
    try {
...
async function lglFetch(url, body, restNonce, isStream, signal = undefined) {
  ...
  return await fetch(`${url}`, {
    method: 'POST',
    headers,
    body: lglStringify(body),
    signal,
  });
}

async function lglFetchUpload(url, file, restNonce, onProgress, params = {}) {
...
const OutputHandler = (props) => {
  const { content, error, isStreaming, baseClass = "lgl-output-handler" } = props;
  const isError = !!error;
...
export { lglHandleRes, lglFetch, lglFetchUpload, randomStr,
  BlinkingCursor, OutputHandler, isEmoji, lglStringify
};