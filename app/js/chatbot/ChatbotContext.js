...
import { applyFilters } from '@app/chatbot/LglAPI';
import { lglHandleRes, lglFetch, randomStr, lglFetchUpload, isEmoji, lglStringify } from '@app/helpers';
import { lglAPI } from '@app/chatbot/LglAPI';
...
export const ChatbotContextProvider = ({ children, ...rest }) => {
  ...
  const localStorageKey = localMemory ? `lgl-chat-${customId || botId}` : null;
  const { cssVariables, iconUrl, aiAvatarUrl, userAvatarUrl, guestAvatarUrl } = useMemo(() => {
    ...
    const cssVariables = Object.keys(shortcodeStyles).reduce((acc, key) => {
      acc[`--lgl-${key}`] = shortcodeStyles[key];
      return acc;
    }, {});
    return {
      cssVariables,
  ...
  const refreshRestNonce = useCallback(async (force = false) => {
    try {
      ...
      setBusyNonce(true);
      const res = await lglFetch(`${restUrl}/lgl/v1/start_session`);
  ...
  useEffect(() => {
    if (customId || botId) {
      const existingChatbotIndex = lglAPI.chatbots.findIndex(
      ...
      if (existingChatbotIndex !== -1) {
        lglAPI.chatbots[existingChatbotIndex] = chatbot;
      } else {
        lglAPI.chatbots.push(chatbot);
      }
    }
  ...
  const saveMessages = useCallback((messages) => {
    ...
    localStorage.setItem(localStorageKey, lglStringify({
      chatId: chatId,
      messages: messages,
    }));
  ...
  const onStartRealtimeSession = useCallback(async () => {
    ...
    const res = await lglFetch(`${restUrl}/lgl-ui/v1/openai/realtime/start`, body, nonce);
  ...
  const onCommitStats = useCallback(async (stats, refId = null) => {
    try {
      const nonce = restNonce ?? await refreshRestNonce();
      const res = await mwaiFetch(`${restUrl}/lgl-ui/v1/openai/realtime/stats`, {
        botId: botId,
        session: sessionId,
        refId: refId || chatId,
        stats: stats
    ...
    async (messages = []) => {
      try {
        ...
        const res = await mwaiFetch(
          `${restUrl}/lgl-ui/v1/openai/realtime/discussions`,
          payload,
          nonce
        );
        const data = await mwaiHandleRes(res);
  ...
  const onRealtimeFunctionCallback = useCallback(async (functionId, functionType, functionName, functionTarget, args) => {
    ...
    if (functionTarget === 'js') {
      ...
      const nonce = restNonce ?? await refreshRestNonce();
      const res = await mwaiFetch(`${restUrl}/lgl-ui/v1/openai/realtime/call`, body, nonce);
      const data = await mwaiHandleRes(res);
      return data;
    }
  ...
  const onSubmit = useCallback(async (textQuery) => {
    ...
    try {
      ...
      const nonce = restNonce ?? await refreshRestNonce();
      const res = await mwaiFetch(`${restUrl}/lgl-ui/v1/chats/submit`, body, nonce, stream);
      const data = await lglHandleRes(res, streamCallback, debugMode ? "CHATBOT" : null);
  ...
  const onFileUpload = async (file, type = "N/A", purpose = "N/A") => {
    try {
      ...
      const url = `${restUrl}/lgl-ui/v1/files/upload`;
      const nonce = restNonce ?? await refreshRestNonce();

      const res = await lglFetchUpload(url, file, nonce, (progress) => {
