...
const ChatbotUI = (props) => {
  ...
  useEffect(() => {
    ...
    const scrollableDiv = document.querySelector('.lgl-window');
    if (scrollableDiv) {
      if (open) {
  ...
  const baseClasses = css('lgl-chatbot', {
    [`lgl-${theme?.themeId}-theme`]: true,
    'lgl-window': isWindow,
    'lgl-bubble': iconBubble,
    'lgl-open': open,
    'lgl-fullscreen': !windowed || (!isWindow && fullscreen),
    'lgl-bottom-left': iconPosition === 'bottom-left',
    'lgl-top-right': iconPosition === 'top-right',
    'lgl-top-left': iconPosition === 'top-left',
  });
  useLayoutEffect(() => {
  ...
  const jsxShortcuts = useMemo(() => {
    ...
    return (
      <div className="lgl-shortcuts">
        {shortcuts.map((action, index) => {
          const { type, data } = action;
          const { label, variant, icon, className } = data ?? {};
          let baseClasses = css('lgl-shortcut', {
            'lgl-success': variant === 'success',
            'lgl-danger': variant === 'danger',
            'lgl-warning': variant === 'warning',
            'lgl-info': variant === 'info',
          });
          if (className) {
          ...
          case 'message': {
            ...
            return (
              <button className={baseClasses} key={index} onClick={onClick}>
                ...
                <div className="lgl-label">{label || "N/A"}</div>
              </button>
            );
          }
          case 'callback': {
  ...
  const jsxBlocks = useMemo(() => {
    ...
    return <div className="lgl-blocks">
      {blocks.map((block, index) => {
        ...
        const baseClasses = css('lgl-block', {
  ...
  const inputClassNames = css('lgl-input', {
    'mwai-dragging': draggingType,
    'mwai-blocked': isBlocked,
  });

  return (
    <TransitionBlock dir="auto" id={`lgl-chatbot-${customId || botId}`}
      ...
      <div className="lgl-content">
        ...
        {!isRealtime && <>
          <div ref={conversationRef} className="lgl-conversation" onScroll={onScroll}>
            {messageList}
            {jsxShortcuts}
          </div>

          {error && <div className="lgl-error" onClick={() => resetError()}>
        ...
        {isRealtime && <div className="lgl-realtime">
          <ChatbotRealtime />
        </div>}

        {needsFooter && <div className="lgl-footer">
          {needTools && <div className="lgl-tools">
            {uploadIconPosition === 'lgl-tools' && <ChatUploadIcon />}
          </div>}
          {textCompliance && (<div className='lgl-compliance'
